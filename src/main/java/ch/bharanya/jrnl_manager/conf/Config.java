...
public class Config {
	 ...
	 */
	private static final File DEFAULT_CONF_PATH = new File(ConfigUtil.getDocBase() + "/res/conf/config.json");
	private static final File TEST_CONF_PATH = new File(ConfigUtil.getDocBase() + "/res/conf/config.test.json");
	private static Config defaultInstance = null;
	private static Config testInstance = null;
	...
	private static Config getTest() {
		if(testInstance == null) { // check here to prevent synchronization.
			synchronized(Config.class) {
				if(testInstance == null) {
					testInstance = new Config(TEST_CONF_PATH);
				}
			}
		}
